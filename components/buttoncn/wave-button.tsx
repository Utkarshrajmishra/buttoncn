  ...
  (
    ...
    return (
      <button
        ref={ref}
        className={cn(
          "group relative flex h-12 min-w-50 cursor-pointer items-center justify-center overflow-hidden rounded-md border p-2 font-semibold text-white duration-500",
          className
        )}
        style={{
      ...
      >
        {waveColors.map((color, index) => (
          <div
            key={index}
            className="absolute z-10 rounded-full transition-all delay-150 duration-500 ease-in-out group-hover:scale-150"
            style={{
