...
const LikeButton = React.forwardRef<HTMLDivElement, LikeButtonProps>(
  (
    { className, initialCount = 68, likedColor = "#ff4458", onLike, ...props },
    ref
  ) => {
    ...
    return (
      <div
        ref={ref}
        className={cn(
          "relative flex h-14 w-36 cursor-pointer overflow-hidden rounded-xl transition-all duration-300 hover:scale-105 active:scale-95",
          className
        )}
        style={{
      ...
      >
        ...
        {particles.map((particle) => (
          <div
            key={particle.id}
            className="pointer-events-none absolute top-1/2 left-[35%] h-1 w-1 rounded-full"
            style={
              {
        ...
        <label
          htmlFor="heart"
          className="relative z-10 flex h-full w-[70%] cursor-pointer items-center justify-center gap-2"
          onClick={handleLike}
        >
          <div className="relative">
            <svg
              ...
              style={{
                ...
                filter: isLiked
                  ? `drop-shadow(0 0 8px ${likedColor}80)`
                  : "none",
        ...
        <div className="absolute right-0 h-full w-[30%] overflow-hidden">
          <div
            className="absolute inset-0 flex items-center justify-center border-l text-base font-semibold transition-all duration-500 ease-out"
            style={{
