...
const splashButtonVariants = cva(
  "relative inline-flex items-center justify-center overflow-hidden rounded-md font-semibold tracking-tight transition-all duration-300 group focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring disabled:opacity-50 disabled:pointer-events-none",
  {
    variants: {
      variant: {
...
const SplashButton = React.forwardRef<HTMLButtonElement, SplashButtonProps>(
  ({ className, variant, size, children, splashColor="orange-600", ...props }, ref) => {
    return (
      <button
        ref={ref}
      ...
      >
        {/* 🔥 Expanding Splash */}
        <span className={cn("absolute w-0 h-0  rounded-full transition-all duration-500 ease-out group-hover:w-56 group-hover:h-56", `bg-${splashColor}`)} />
